  ...
  return (
    <Grid columns={1} rowGap={'20px'}>
      ...
      {booking.priceType === 'daily' ? (
        <Box m="0"><ListDates dates={booking.reservations} /></Box>
      ) : (
        <>
          <Cell>
            <LeftTitleStyled>Period</LeftTitleStyled><br/>
...
const PendingBooking = ({
  ...
  dispatch
    ...
    <Grid columns={1} rowGap={'20px'}>
      ...
      />
      <Grid columns={2}>
        <Cell md={6}><Button fluid error onClick={() => _onCancelBooking(booking, dispatch)}>Cancel</Button></Cell>
        <Cell md={6}><Button fluid onClick={() => _onContinueBooking(booking, dispatch)}>Continue</Button></Cell>
      </Grid>
    </Grid>
  )
;
